...
thread1 = threading.Thread(target=sense.main)
...
class PurchaseEventHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        if event.src_path.endswith('user_events.json'):
...
class PurchaseMonitorService(win32serviceutil.ServiceFramework):
    _svc_name_ = "PurchaseMonitorService_003"
    _svc_display_name_ = "Purchase Monitor Service 003"
